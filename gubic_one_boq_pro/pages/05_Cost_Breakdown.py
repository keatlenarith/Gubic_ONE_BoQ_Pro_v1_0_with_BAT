...
from modules.ui_helpers import inject_css, page_header, require_data, filter_dataframe, fit_dataframe
...
with tabs[0]: fit_dataframe(cost_breakdown(filtered), use_container_width=True)
with tabs[1]: fit_dataframe(package_ranking(filtered), use_container_width=True)
with tabs[2]: fit_dataframe(section_ranking(filtered), use_container_width=True)
with tabs[3]:
    st.plotly_chart(top_items_bar(filtered, 20), use_container_width=True)
    fit_dataframe(top_items(filtered, 50), use_container_width=True)
with tabs[4]: fit_dataframe(validation_report(filtered), use_container_width=True)
